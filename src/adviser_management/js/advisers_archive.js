$(document).ready(function () {
  $("#archiveAdvisersTable").DataTable({
    paging: true,
    searching: true,
    info: true,
    lengthChange: true,
...
$(document).on("click", ".recover-btn", function () {
  var adviserId = $(this).data("id"); // Get the adviser_id from the button's data-id attribute
  $("#recover_adviser_id").val(adviserId); // Set the value of the hidden input
});

$("#confirmRecoverBtn").on("click", function () {
  var adviserId = $("#recover_adviser_id").val(); // Get the item ID from the hidden input field

  // Send an AJAX request to delete the item
  $.ajax({
    url: "recover_adviser.php", // PHP file to handle deletion
    type: "POST",
