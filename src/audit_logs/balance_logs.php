...
<body>
    ...
    <div class="container p-4" style="margin-top: 50px;">
        <h2><span class="text-warning fw-bold me-2">|</span> Balance History</h2>

        <!-- Balance History Table -->
        <div class="table-responsive">
            <table class="table mt-4" id="balanceHistoryTable">
                <thead>
                    <tr>
                        <th>Organization</th>
                        <th>Amount</th>
                ...
                <tbody>
                    ...
                    $query = "
                        SELECT h.balance, h.updated_at, u.first_name, u.last_name, o.organization_name
                    ...
                    while ($row = mysqli_fetch_assoc($result)) {
                        ...
                        $amount = number_format($row['balance'], 2);  // Format amount as currency
    ...
    <div class="container p-4" style="margin-top: 50px;">
        <h2><span class="text-warning fw-bold me-2">|</span> Income History</h2>

        <!-- Income History Table -->
        <div class="table-responsive">
            <table class="table mt-4" id="incomeHistoryTable">
                <thead>
                    <tr>
                        <th>Organization</th>
                        <th>Amount</th>
                ...
                <tbody>
                    <?php
                    // Fetch income history from the database
                    $query = "
                        SELECT h.income, h.updated_at, u.first_name, u.last_name, o.organization_name
                        FROM income_history h
                        JOIN organizations o ON h.organization_id = o.organization_id
                        JOIN users u ON h.created_by = u.user_id
                    ";
                    $result = mysqli_query($conn, $query);
                    ...
                    while ($row = mysqli_fetch_assoc($result)) {
                        $organization = $row['organization_name'];  // Organization name
                        $amount = number_format($row['income'], 2);  // Format amount as currency
                        $updated_at = date('Y-m-d H:i:s', strtotime($row['updated_at']));  // Format date
                        $created_by = $row['first_name'] . ' ' . $row['last_name'];  // User's full name
                        ?>
                        <tr>
    ...
    <div class="container p-4" style="margin-top: 50px;">
        <h2><span class="text-warning fw-bold me-2">|</span> Expense History</h2>

        <!-- Expense History Table -->
        <div class="table-responsive">
            <table class="table mt-4" id="expenseHistoryTable">
                <thead>
                    <tr>
                        <th>Organization</th>
                        <th>Amount</th>
                ...
                <tbody>
                    ...
                    $query = "
                        SELECT h.expense, h.updated_at, u.first_name, u.last_name, o.organization_name
                    ...
                    while ($row = mysqli_fetch_assoc($result)) {
                        $organization = $row['organization_name'];  // Organization name
                        $amount = number_format($row['expense'], 2);  // Format amount as currency
                        $updated_at = date('Y-m-d H:i:s', strtotime($row['updated_at']));  // Format date
                        $created_by = $row['first_name'] . ' ' . $row['last_name'];  // User's full name
                        ?>
                        <tr>
