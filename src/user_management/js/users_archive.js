...
$(document).on("click", ".recover-btn", function () {
  var userId = $(this).data("id"); // Get the user_id from the button's data-id attribute
  $("#recover_user_id").val(userId); // Set the value of the hidden input
});

$("#confirmRecoverBtn").on("click", function () {
  var userId = $("#recover_user_id").val(); // Get the item ID from the hidden input field
